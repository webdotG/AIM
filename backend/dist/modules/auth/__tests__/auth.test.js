...
describe('Auth Module - Complete Test Suite', () => {
    beforeEach(async () => {
        await pool_1.pool.query("DELETE FROM users WHERE login LIKE 'test_%' OR login LIKE 'testuser%' OR login LIKE 'test-%'");
    });
    ...
    describe('GET /api/v1/auth/generate-password', () => {
        ...
        it('should generate different passwords on multiple requests', async () => {
            const response1 = await (0, supertest_1.default)(index_1.default).get('/api/v1/auth/generate-password');
            const response2 = await (0, supertest_1.default)(index_1.default).get('/api/v1/auth/generate-password');
    ...
    describe('Security Tests', () => {
        ...
        it('should sanitize SQL injection attempts', async () => {
            ...
            const result = await pool_1.pool.query('SELECT COUNT(*) FROM users');
            expect(result.rows).toBeDefined();
        });
        it('should handle concurrent registrations correctly', async () => {
            const promises = Array(5).fill(null).map((_, i) => (0, supertest_1.default)(index_1.default)
                .post('/api/v1/auth/register')
                .send({
