...
class AnalyticsRepository extends BaseRepository_1.BaseRepository {
    ...
    async getEntriesByMonth(userId, months = 12) {
       ...
       ORDER BY month ASC, count DESC`, [userId]);
        return result.rows;
    }
    async getEmotionDistribution(userId) {
       ...
       WHERE n.user_id = $1 AND n.deleted_at IS NULL
       GROUP BY e.category
       ORDER BY occurrence_count DESC`, [userId]);
        return result.rows;
    }
    ...
    async getStreaks(userId) {
      ...
      LIMIT 1`, [userId]);
        const lastEntry = await this.pool.query('SELECT MAX(created_at)::date AS last_date FROM nodes WHERE user_id = $1 AND deleted_at IS NULL', [userId]);
