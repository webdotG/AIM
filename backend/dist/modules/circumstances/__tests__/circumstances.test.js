...
describe('Circumstances Module', () => {
    ...
    beforeEach(async () => {
        ...
        await pool_1.pool.query('DELETE FROM circumstances WHERE user_id = $1', [testUser.id]);
    });
    afterEach(async () => {
    ...
    describe('GET /api/v1/circumstances', () => {
        it('should list all circumstances for user', async () => {
            await pool_1.pool.query(`INSERT INTO circumstances (user_id, weather, temperature, notes) 
         VALUES ($1, $2, $3, $4), ($5, $6, $7, $8)`, [testUser.id, 'Sunny', 25, 'Nice day', testUser.id, 'Rainy', 15, 'Cold day']);
        ...
        it('should not show circumstances from other users', async () => {
            ...
            await pool_1.pool.query(`INSERT INTO circumstances (user_id, weather, temperature) 
         VALUES ($1, $2, $3), ($4, $5, $6)`, [otherUser.id, 'Snowy', 0, testUser.id, 'Sunny', 25]);
            const response = await (0, supertest_1.default)(index_1.default)
                .get('/api/v1/circumstances')
                .set('Authorization', test_helpers_1.TestHelpers.authHeader(authToken))
                .expect(200);
    ...
    describe('GET /api/v1/circumstances/:id', () => {
        let circumstanceId;
        beforeEach(async () => {
            const result = await pool_1.pool.query(`INSERT INTO circumstances (user_id, weather, temperature) 
         VALUES ($1, $2, $3) RETURNING id`, [testUser.id, 'Sunny', 25]);
        ...
        it('should not allow access to other user circumstances', async () => {
            ...
            const result = await pool_1.pool.query(`INSERT INTO circumstances (user_id, weather, temperature) 
         VALUES ($1, $2, $3) RETURNING id`, [otherUser.id, 'Foggy', 10]);
    ...
    describe('PUT /api/v1/circumstances/:id', () => {
        ...
        beforeEach(async () => {
            const result = await pool_1.pool.query(`INSERT INTO circumstances (user_id, weather, temperature) 
       VALUES ($1, $2, $3) RETURNING id`, [testUser.id, 'sunny', 25] // Используем нижний регистр
            );
            circumstanceId = result.rows[0].id;
        });
        ...
        it('should not allow updating other user circumstances', async () => {
            ...
            const result = await pool_1.pool.query(`INSERT INTO circumstances (user_id, weather, temperature) 
       VALUES ($1, $2, $3) RETURNING id`, [otherUser.id, 'foggy', 10] // нижний регистр
            );
            const otherCircumstanceId = result.rows[0].id;
            const response = await (0, supertest_1.default)(index_1.default)
...
//# sourceMappingURL=circumstances.test.js.map