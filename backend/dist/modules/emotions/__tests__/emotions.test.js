...
describe('Emotions Module', () => {
    ...
    beforeEach(async () => {
        ...
        testEntry = await test_factories_1.TestFactories.createEntry(testUser.id, {
            entry_type: 'dream',
    ...
    describe('POST /api/v1/emotions/entry/:entryId', () => {
        it('should attach emotions to entry', async () => {
            ...
            const emotionResult = await pool_1.pool.query('SELECT id FROM emotions WHERE name_en = $1 LIMIT 1', ['Joy']);
        ...
        it('should reject invalid intensity', async () => {
            // Получаем реальную эмоцию из БД
            const emotionResult = await pool_1.pool.query('SELECT id FROM emotions LIMIT 1');
            const emotionId = emotionResult.rows[0].id;
            const response = await (0, supertest_1.default)(index_1.default)
                .post(`/api/v1/emotions/entry/${testEntry.id}`)
                .set('Authorization', test_helpers_1.TestHelpers.authHeader(authToken))
    ...
    describe('GET /api/v1/emotions/entry/:entryId', () => {
        it('should get emotions for entry', async () => {
            // Получаем реальную эмоцию из БД
            const emotionResult = await pool_1.pool.query('SELECT id, name_en FROM emotions LIMIT 1');
    ...
    describe('GET /api/v1/emotions/stats', () => {
        beforeEach(async () => {
            ...
            const entry1 = await test_factories_1.TestFactories.createEntry(testUser.id);
            const entry2 = await test_factories_1.TestFactories.createEntry(testUser.id);
            // Получаем реальные эмоции из БД
            const emotions = await pool_1.pool.query('SELECT * FROM emotions LIMIT 2');
