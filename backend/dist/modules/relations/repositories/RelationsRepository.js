...
exports.RelationsRepository = void 0;
const BaseRepository_1 = require("../../../shared/repositories/BaseRepository");
class RelationsRepository extends BaseRepository_1.BaseRepository {
    constructor(pool) {
        super(pool);
    }
    async findById(id) {
        const result = await this.pool.query(`SELECT * FROM entry_relations WHERE id = $1`, [id]);
    ...
    async getForEntry(entryId) {
      ...
      ORDER BY er.created_at DESC`, [entryId]);
    ...
    // Создать связь
    async create(data) {
        const result = await this.pool.query(`INSERT INTO entry_relations (from_entry_id, to_entry_id, relation_type, description)
       VALUES ($1, $2, $3, $4)
       RETURNING *`, [data.from_entry_id, data.to_entry_id, data.relation_type, data.description || null]);
        return result.rows[0];
    }
    // Удалить связь
    async delete(id) {
        const result = await this.pool.query(`DELETE FROM entry_relations WHERE id = $1 RETURNING id`, [id]);
        return result.rows[0];
    }
    // Получить цепочку связей (граф) - рекурсивный CTE
    async getChain(entryId, maxDepth = 10, direction = 'both') {
        let query = '';
    ...
    async hasCycle(fromId, toId) {
        const result = await this.pool.query(`WITH RECURSIVE path AS (
      ...
      SELECT EXISTS(SELECT 1 FROM path WHERE current_id = $2) as has_cycle`, [toId, fromId]);
