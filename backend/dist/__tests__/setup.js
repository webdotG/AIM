...
async function createFullSchema(pool) {
    // Extensions
...
async function insertReferenceData(pool) {
    ...
    for (const [code, name] of nodeTypes) {
        await pool.query(`INSERT INTO node_types(code, name) VALUES ($1, $2) ON CONFLICT (code) DO NOTHING`, [code, name]);
    }
    ...
    for (const [code, name] of edgeTypes) {
        await pool.query(`INSERT INTO edge_types(code, name) VALUES ($1, $2) ON CONFLICT (code) DO NOTHING`, [code, name]);
    }
    ...
    for (const [code, nameEn, nameRu, category] of emotions) {
        await pool.query(`INSERT INTO emotions(code, name_en, name_ru, category)
       VALUES ($1, $2, $3, $4) ON CONFLICT (code) DO NOTHING`, [code, nameEn, nameRu, category]);
    }
}
