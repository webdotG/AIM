...
beforeAll(async () => {
    ...
    try {
        ...
        const dbCheck = await adminPool.query("SELECT 1 FROM pg_database WHERE datname = 'dream_journal_test'");
      ...
      `);
        ...
        exports.testPool = testPool = new pg_1.Pool({
            host: process.env.DB_HOST,
            port: parseInt(process.env.DB_PORT || '5432'),
            user: process.env.DB_USER,
            password: process.env.DB_PASSWORD,
...
async function insertBaseEmotions(pool) {
    ...
    for (const [name_en, name_ru, category] of emotions) {
        try {
            await pool.query(`INSERT INTO emotions (name_en, name_ru, category) 
         VALUES ($1, $2, $3) 
         ON CONFLICT (name_en) DO NOTHING`, [name_en, name_ru, category]);
            insertedCount++;
        }
        catch (error) {
