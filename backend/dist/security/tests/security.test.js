...
describe('Security Sanitizers', () => {
    describe('SQLInjectionSanitizer', () => {
        it('should detect and sanitize SQL injection attempts', () => {
            const sanitizer = new SQLInjectionSanitizer_1.SQLInjectionSanitizer();
        ...
        it('should sanitize SQL injection patterns', () => {
            ...
            expect(result2).not.toContain("SELECT");
        });
    });
    ...
    describe('CommandInjectionSanitizer', () => {
        it('should detect and sanitize command injection attempts', () => {
            const sanitizer = new CommandInjectionSanitizer_1.CommandInjectionSanitizer();
        ...
        it('should sanitize command injection patterns', () => {
            ...
            expect(result1.length).toBeLessThan('test; cat /etc/passwd'.length);
        });
    });
    describe('PathTraversalSanitizer', () => {
        it('should detect and sanitize path traversal attempts', () => {
            const sanitizer = new PathTraversalSanitizer_1.PathTraversalSanitizer();
    ...
    describe('TrimSanitizer', () => {
        it('should trim whitespace from strings', () => {
            const sanitizer = new TrimSanitizer_1.TrimSanitizer();
    ...
    describe('LengthSanitizer', () => {
        it('should enforce maximum length', () => {
            const sanitizer = new LengthSanitizer_1.LengthSanitizer({ maxLength: 10 });
            const result = sanitizer.sanitize('Hello World This is a long string');
            expect(result.length).toBeLessThanOrEqual(10);
        });
        it('should handle minimum length in strict mode', () => {
            const sanitizer = new LengthSanitizer_1.LengthSanitizer({ minLength: 5, strict: true });
            expect(() => sanitizer.sanitize('Hi')).toThrow();
        });
    });
    describe('Presets', () => {
        it('userInputPreset should sanitize user input', () => {
            const pipeline = (0, userInputPreset_1.createUserInputPreset)({ debug: false });
            ...
            expect(result.email).not.toContain('OR');
            expect(result.search).not.toContain('..');
        });
        it('apiPreset should sanitize API data', async () => {
            const pipeline = (0, apiPreset_1.createApiPreset)({ debug: false });
            const maliciousInput = {
            ...
            expect(result.query.search).not.toContain('DROP');
        });
        it('searchPreset should sanitize search queries', () => {
            const pipeline = (0, searchPreset_1.createSearchPreset)({ debug: false });
