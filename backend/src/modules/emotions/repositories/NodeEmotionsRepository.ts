...
export class NodeEmotionsRepository extends BaseRepository {
  ...
  async findByNodeId(nodeId: string) {
    const result = await this.pool.query(
      `SELECT ne.*, e.code AS emotion_code, e.name_ru AS emotion_name_ru, e.name_en AS emotion_name_en, e.category AS emotion_category
       FROM node_emotions ne
       JOIN emotions e ON e.id = ne.emotion_id
       WHERE ne.node_id = $1
       ORDER BY ne.intensity DESC`,
      [nodeId]
    );
    return result.rows;
  }

  async replaceForNode(nodeId: string, emotions: { emotion_id: number; intensity: number }[]) {
    ...
    try {
      ...
      if (emotions.length > 0) {
        for (const e of emotions) {
          await client.query(
            'INSERT INTO node_emotions (node_id, emotion_id, intensity) VALUES ($1, $2, $3)',
            [nodeId, e.emotion_id, e.intensity]
          );
        }
      }
  ...
  async removeFromNode(nodeId: string) {
    const result = await this.pool.query(
      'DELETE FROM node_emotions WHERE node_id = $1 RETURNING id',
      [nodeId]
    );
  ...
  async getDistribution(userId: number, granularity: 'day' | 'week' | 'month' = 'day') {
    ...
    const result = await this.pool.query(
      `SELECT
       ...
       ORDER BY period DESC, occurrence_count DESC`,
      [userId]
    );
    return result.rows;
  }
