...
export class EdgesRepository extends BaseRepository {
  constructor(pool: Pool) {
    super(pool);
  }

  async findByNode(nodeId: string, direction: 'outgoing' | 'incoming' | 'both' = 'both') {
    let condition = '';
    ...
    const result = await this.pool.query(
      `SELECT e.*, et.code AS edge_type_code, et.name AS edge_type_name
       FROM edges e
  ...
  async create(fromNodeId: string, toNodeId: string, edgeTypeCode: string, confidence: number | null = null, weight: number | null = null, notes: string | null = null) {
    const result = await this.pool.query(
      `INSERT INTO edges (from_node_id, to_node_id, edge_type_id, confidence, weight, notes)
       SELECT $1, $2, et.id, $3, $4, $5
       FROM edge_types et WHERE et.code = $6
       RETURNING id, from_node_id, to_node_id, edge_type_id, confidence, weight, created_at, notes, deleted_at`,
      [fromNodeId, toNodeId, confidence, weight, notes, edgeTypeCode]
    );
  ...
  async findChain(startNodeId: string, direction: 'forward' | 'backward' | 'both' = 'both', depth: number = 10) {
    let directionCond = '';
    ...
    const result = await this.pool.query(
      ...
      SELECT * FROM traversal WHERE depth > 0 ORDER BY depth, node`,
      [startNodeId, depth]
    );
    return result.rows;
  }
  ...
  async getGraphData(userId: number) {
    const nodesResult = await this.pool.query(
      `SELECT n.id, n.node_type_id, nt.code AS node_type_code, n.title
       FROM nodes n
    ...
    const edgesResult = await this.pool.query(
      `SELECT e.id, e.from_node_id, e.to_node_id, et.code AS edge_type_code, e.confidence, e.weight
       FROM edges e
       JOIN edge_types et ON et.id = e.edge_type_id
       WHERE e.deleted_at IS NULL
         AND EXISTS (SELECT 1 FROM nodes n WHERE n.id = e.from_node_id AND n.user_id = $1 AND n.deleted_at IS NULL)
         AND EXISTS (SELECT 1 FROM nodes n WHERE n.id = e.to_node_id AND n.user_id = $1 AND n.deleted_at IS NULL)`,
      [userId]
    );
  ...
  async getMostConnected(userId: number, limit: number = 10) {
    const result = await this.pool.query(
      `SELECT n.id, n.title, nt.code AS node_type_code,
