...
describe('BodyStates Module', () => {
  ...
  beforeEach(async () => {
    ...
    await pool.query('DELETE FROM body_states WHERE user_id = $1', [testUser.id]);
  });
 ...
 describe('GET /api/v1/body-states', () => {
  it('should list all body states for user', async () => {
    await pool.query(
      `INSERT INTO body_states (user_id, health_points, energy_points, location_name) 
       VALUES ($1, $2, $3, $4), ($5, $6, $7, $8)`,
      [testUser.id, 80, 70, 'Home', testUser.id, 90, 85, 'Work']
    );
  ...
  it('should not show body states from other users', async () => {
    ...
    await pool.query(
      `INSERT INTO body_states (user_id, health_points, energy_points) 
       VALUES ($1, $2, $3)`,
      [otherUser.id, 50, 60]
    );
...
it('should delete body state', async () => {
  ...
  if (data && data.body_states) {
    ...
    const deletedState = data.body_states.find((s: any) => s.id === bodyStateId);
