...
export class SkillsService {
  ...
  async getAllSkills(userId: number, filters: any = {}) {
  ...
  async createSkill(data: any, userId: number) {
    // Валидация
  ...
  async updateSkill(id: number, updates: any, userId: number) {
  ...
  async addProgress(skillId: number, progressData: any, userId: number) {
    ...
    const updateResult = await this.skillsRepository.updateExperienceAndLevel(
      skillId, 
      progressData.experience_gained || 10
    );
  ...
  async getProgressHistory(skillId: number, userId: number) {
    const skill = await this.skillsRepository.findById(skillId, userId);
  ...
  async getTopSkills(userId: number, limit: number = 10) {
    return await this.skillsRepository.getTopSkills(userId, limit);
  }

  private validateSkillData(data: any) {
    ...
    if (data.current_level !== undefined) {
      if (data.current_level < 1 || data.current_level > 100) {
        throw new AppError('current_level must be between 1 and 100', 400);
      }
    }

    if (data.experience_points !== undefined) {
      if (data.experience_points < 0) {
        throw new AppError('experience_points cannot be negative', 400);
      }
    }
  }
}