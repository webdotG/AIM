...
constructor(pool: Pool) {
  ...
  async addProgress(data: ProgressData) {
    const result = await this.pool.query(
  ...
  async getProgressHistory(skillId: number, limit: number = 50) {
    const result = await this.pool.query(
      `SELECT 
        sp.*,
  ...
  async getTotalExperienceGained(skillId: number) {
    const result = await this.pool.query(
      `SELECT SUM(experience_gained) as total FROM skill_progress WHERE skill_id = $1`,
      [skillId]
    );
  ...
  async updateExperienceAndLevel(skillId: number, experienceGained: number) {
    // Получаем текущий навык
    const skill = await this.pool.query(
      `SELECT current_level, experience_points FROM skills WHERE id = $1`,
      [skillId]
    );
